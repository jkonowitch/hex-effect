...
export const make = (
  ...
  Effect.gen(function* () {
    ...
    const makeConnection = Effect.gen(function* () {
      const sdk = Libsql.createClient(options as Libsql.Config);
      ...
      const run = (sql: string, params: ReadonlyArray<Statement.Primitive> = []) =>
        Effect.tryPromise({
          try: () =>
            sdk
              .execute({ sql, args: params as Array<Libsql.InValue> })
              .then((results) => results.rows),
          catch: (cause) => new SqlError({ cause, message: 'Failed to execute statement' })
        });

      const runRaw = (sql: string, params: ReadonlyArray<Statement.Primitive> = []) =>
        Effect.tryPromise({
          try: () => sdk.execute({ sql, args: params as Array<Libsql.InValue> }),
          catch: (cause) => new SqlError({ cause, message: 'Failed to execute statement' })
        });
      ...
      return identity<LibsqlConnection>({
        ...
        executeValues(sql, params) {
          return Effect.map(run(sql, params), (rows) =>
            ...
            rows.map((row) => Array.from(row) as Array<any>)
          );
        },
...
export const layer = (
  ...
  Layer.scopedContext(
    Config.unwrap(config).pipe(
      Effect.flatMap(make),
      Effect.map((client) =>
        Context.make(LibsqlClient, client).pipe(Context.add(Client.SqlClient, client))
      )
    )
  );
