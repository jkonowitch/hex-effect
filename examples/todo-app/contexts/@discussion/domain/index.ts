...
export class Discussion extends Schema.TaggedClass<Discussion>()('Discussion', {
  ...
  public static create(projectId: typeof ProjectId.Type) {
    return Effect.gen(function* () {
      const discussion = Discussion.make({
        id: yield* Effect.serviceFunctions(DiscussionRepository).nextId(),
        projectId
      });
      yield* Effect.serviceFunctions(DiscussionDomainPublisher).publish(
        DiscussionCreatedEvent.make({ discussionId: discussion.id })
      );
      return discussion;
    });
  }
  ...
  public static create(discussionId: typeof DiscussionId.Type, content: string) {
    return Effect.gen(function* () {
      const comment = Comment.make({
        id: yield* Effect.serviceFunctions(CommentRepository).nextId(),
        discussionId,
        content
      });
      yield* Effect.serviceFunctions(DiscussionDomainPublisher).publish(
        CommentCreatedEvent.make({ commentId: comment.id })
      );
      return comment;
    });
  }
...
export class DiscussionRepository extends Context.Tag('DiscussionRepository')<
...
export class DiscussionDomainPublisher extends Context.Tag('DiscussionDomainPublisher')<
  DiscussionDomainPublisher,
  {
